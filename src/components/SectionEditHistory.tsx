...
interface SectionEditHistoryProps {
  sectionId: string;
}
...
function DiffView({ oldText, newText }: { oldText: string; newText: string }) {
  ...
  return (
    <div className="text-foreground/80 whitespace-pre-wrap leading-relaxed text-xs">
      {diff.map((part, index) => (
        <span
          key={index}
          className={cn(
...
function EditHistoryItem({ 
  ...
  return (
    <div className="text-xs bg-secondary/50 rounded-md p-2 space-y-1">
      ...
      {statusChanged && (
        <div className="text-muted-foreground">
          <span className="line-through">{entry.previous_status}</span>
          {' → '}
      ...
      {contentChanged && (
        <div className="space-y-1">
          ...
          {showDetails && (
            <div className="bg-secondary/30 border border-border/50 rounded p-2 max-h-32 overflow-y-auto">
...
export function SectionEditHistory({ sectionId }: SectionEditHistoryProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  const { data: history = [] } = useQuery({
    queryKey: ['section-edit-history', sectionId],
    queryFn: async () => {
