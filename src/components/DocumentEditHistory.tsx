...
function EditHistoryItem({ 
  ...
  const titleChanged = entry.previous_title !== entry.new_title;
    ...
    <div className="text-xs bg-secondary/50 rounded-lg p-3 space-y-2">
      ...
      {titleChanged && (
        <div className="text-muted-foreground">
          Title: <span className="line-through">{entry.previous_title}</span>
          {' → '}
          <span className="text-primary">{entry.new_title}</span>
        </div>
      )}
...
export function DocumentEditHistory({ documentId }: DocumentEditHistoryProps) {
  ...
  const { data: history = [] } = useQuery({
    queryKey: ['document-edit-history', documentId],
    queryFn: async () => {
